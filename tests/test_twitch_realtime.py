...
def test_twitch_device_sign_in_fills_broadcaster_and_tokens(tmp_path, monkeypatch):
    ...
    def post_form(url, values):
        ...
        return {
            "access_token": "twitch-access", "refresh_token": "twitch-refresh",
            "expires_in": 14400,
        }
    ...
    row = service.db.frame(
        "SELECT twitch_access_token,twitch_refresh_token FROM live_integration_settings WHERE id=1"
    ).iloc[0]
